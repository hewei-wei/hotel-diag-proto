...
- 新增酒店。
- 将酒店分配给运营人员。
- 在酒店管理页的“活动配置”弹窗中，查看活动数据并打开或关闭活动的用户端诊断资格；活动配置为平台通用配置，适用于全部酒店。
...
活动配置为平台通用配置，适用于全部酒店，不作为独立页面；入口为酒店管理页的“活动配置”按钮，点击按钮弹窗展示配置内容。
...
酒店管理页由全局框架、页面头部、前置条件提示、检索筛选和酒店列表五部分组成，页面头部右侧提供“活动配置”按钮。左侧导航在各页面保持同一结构：品牌区、“酒店管理”菜单和“系统管理”分组；进入具体酒店后，酒店相关页面（诊断工作台、诊断任务、诊断日志）通过右侧内容区顶部的二级导航切换，二级导航持续展示当前酒店名称，并提供返回酒店管理页的入口。酒店管理页全部元素如下：
...
| 页面头部 | 活动配置按钮 | 页面头部右侧展示“活动配置”按钮；点击按钮以弹窗展示活动配置，配置为平台通用配置，适用于全部酒店 |
| 页面头部 | 标题 | 展示“酒店管理” |
...
- 入口为酒店管理页的“活动配置”按钮，点击弹窗展示配置内容；配置为平台通用配置，适用于全部酒店。
- 活动状态分为“已启用”“已停用”“已下线”：已启用为开启诊断；已停用为关闭诊断；已下线不再进入用户端诊断及报名候选。
- 未下线活动可开启或关闭诊断（已启用与已停用互切）；新活动默认已停用。
- 下线：点击“下线”后二次确认，确认后状态为已下线；已下线活动不再允许诊断，且不展示开启/关闭诊断入口；下线不修改抖音活动状态。
- 重新上线：已下线活动展示“重新上线”按钮，点击后二次确认；确认上线后恢复开启/关闭诊断能力，并恢复下线前的开启/关闭状态。
- 历史活动本次未出现时不删除，自动标记为已下线；活动重新出现时恢复在线并保留原开启/关闭状态。
...
- 页面名称为“诊断日志”，以“检查路径 → 诊断次数”两级结构组织。
- 页面顶部展示全局汇总统计：累计诊断次数、成功次数、失败次数。
- 记录按检查路径分类归组，同一路径的全部历史诊断归入同一组。
- 分组头部展示该路径的检查路径、累计诊断次数、累计成功数、累计失败数，以及最近一次诊断结果与时间。
- 展开分组后按诊断次数展示明细：每次诊断展示诊断次序、诊断时间、诊断结果（通过或不通过）、本次生成任务数，以及截至该次诊断的累计诊断次数、累计成功数、累计失败数，按时间倒序排列（最新一次诊断的累计值即全量汇总）。
...
- 酒店管理页页面头部右侧展示“活动配置”按钮；点击按钮以弹窗展示活动配置，配置为平台通用配置，适用于全部酒店。
- 授权入口位于对应酒店行。
...
- 页面顶部展示全局汇总统计卡：累计诊断次数、成功次数、失败次数。
- 日志按检查路径分组展示，分组头部展示检查路径、累计诊断次数、累计成功数、累计失败数、最近一次诊断结果和时间。
- 展开分组后按诊断次数展示每次诊断的次序、时间、结果、生成任务数，以及截至该次诊断的累计诊断次数、累计成功数、累计失败数。
...
- 诊断日志按检查路径分组、按诊断次数展示每次诊断结果、生成任务数与累计统计。
- 任务不展示抖音不存在的字段。
- 携程无值时 AI 不自行补写。
...
| 首次/最近采集时间 | 用于同步记录 |
| 管理员开关 | 已启用/已停用/已下线，数据同步时保留 |

**管理员规则**

- 入口：酒店管理页“活动配置”弹窗（平台通用配置，适用于全部酒店）。
- 活动状态：已启用（开启诊断）/ 已停用（关闭诊断）/ 已下线（不进入用户端诊断与报名候选）。
- 新活动首次进入后台时默认已停用。
- 未下线活动可开启或关闭诊断（已启用与已停用互切）。
- 下线：点击“下线”后二次确认，确认后置为已下线；已下线活动不展示、不诊断、不生成任务，且不展示开启/关闭诊断入口；不改变抖音活动状态。
- 重新上线：已下线活动展示“重新上线”按钮，点击后二次确认；确认上线后恢复开启/关闭诊断能力，并恢复下线前的开启/关闭状态。
- 同一活动业务字段更新时以最新采集值覆盖当前已存业务字段，并保留活动状态。
- 历史活动本次未出现时不删除，自动标记为已下线；重新出现时恢复在线并保留原开启/关闭状态。

**用户端规则**

- 只展示“在线＋已启用＋立即报名”的活动；“参与详情”视为已参与，不进入任务。
