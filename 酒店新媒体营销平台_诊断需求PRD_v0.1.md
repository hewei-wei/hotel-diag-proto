...
- 在权限管理中为每个子账号配置菜单权限和任务权限。
- 在账号管理中恢复子账号被关闭的任务权限。
- 不受权限限制，可访问全部菜单与全部诊断任务。
...
- 页面头部提供“新增子账号”按钮，弹出新增弹窗：填写员工姓名和登录手机号；确认后创建子账号并默认开通全部菜单权限，任务权限默认全部开通。
...
- 页面头部提供子账号切换，权限配置按所选子账号生效。
- 菜单权限区按菜单页面逐项开关，包括：酒店管理、酒店管理-诊断工作台、酒店管理-诊断任务、酒店管理-诊断日志。
- 任务权限区按诊断任务节点平铺展示，逐节点开关；开关结果与账号管理中的任务权限配置一致。
- 管理员自身不受权限限制。
...
- 权限管理含两块权限：
  ...
  2. 任务权限：即每一个诊断任务的节点权限，平铺展示。
- 权限按子账号独立配置；管理员自身不受权限限制。
