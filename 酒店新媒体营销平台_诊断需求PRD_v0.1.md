...
- 将任务处理为“以后不再提醒”后，该任务节点的任务权限自动关闭；如后续仍需处理，由管理员在用户管理中重新开启。
...
- 线下任务的完成记录。
- 诊断日志。
- 用户管理：账户全生命周期管理（新增、编辑、禁用、删除、重置密码），并为运营账户配置酒店与任务权限。
- 角色管理与菜单管理：角色绑定菜单、按钮、接口三类资源；资源树统一维护，资源开关即时生效。
- 我的信息：账户信息查看与修改密码（密码历史检查，改密后强制重新登录）。
...
酒店管理页页面头部提供“新增酒店”按钮：点击后弹窗输入酒店名称即可新增；新增酒店默认抖音来客和携程官网均为未授权，需完成授权后才可诊断。“新增酒店”按钮受菜单管理中对应按钮资源的权限控制。
...
- 以后不再提醒：点击后弹出二次确认弹窗，确认后关闭本次任务并转为已处理，任务状态为“已忽略”；同时关闭该酒店该节点后续自动诊断提醒，直至主动恢复。运营人员确认时，其该任务节点的任务权限同步关闭，后续同节点任务不可再处理，需管理员在用户管理中重新开启；管理员确认时不涉及任务权限变化。
...
系统管理采用“账号 — 角色 — 资源”三层关系：账户绑定角色（多对多），角色绑定菜单、按钮、接口三类资源（多对多），账户的可用权限取其全部角色的资源并集；菜单资源控制导航显隐，按钮资源控制操作显隐，接口资源约束后端调用，诊断任务节点作为“诊断工作台-诊断任务”菜单下的按钮级资源参与判定。
...
- 页面头部提供“新增账户”按钮，弹窗字段为：账户名、密码、确认密码、用户名称、角色（多选）、账户状态。校验规则：
  ...
  - 密码 8-16 位，必须同时包含数字、大写字母、小写字母和特殊符号，禁止空格；确认密码必须与密码一致。
  - 用户名称为展示名，与账户名分离，必填。
  - 角色至少选择一个，可选范围不含内置超级管理员角色。
- 行操作提供：编辑（用户名称、角色、账户状态；密码不回显）、配置酒店、任务权限、重置密码、删除。
- 内置管理员账户展示“内置账户”标签，不可禁用、不可删除。
- 删除、重置密码均弹出二次确认弹窗；重置成功提示“重置成功”。
- 禁用账户即时生效：被禁用账户无法登录；人员离职优先禁用保留记录，删除仅用于误建账户。
- “配置酒店”弹窗逐酒店勾选，展示酒店名称和门店副名称；保存后配置的酒店出现在该账户登录后的酒店管理页面，可查看和诊断（数据权限按账户维度配置）。
- “任务权限”弹窗按诊断任务节点平铺展示，逐节点开关；用于将账户被自动关闭的任务权限重新开启。
- 账户将任务处理为“以后不再提醒”后，该任务节点权限自动关闭，列表对应账户可在“任务权限”中恢复。

### 15.6 角色管理页

- 入口位于左侧导航“系统管理”分组，仅管理员可进入。
- 列表列依次为：角色名称、状态（开关）、备注、菜单权限（展示已授权菜单摘要）、操作（编辑、删除）。
- 页面头部提供“新增角色”按钮，弹窗字段为：角色名称（必填、唯一）、状态、备注、菜单权限树（核心控件）。
- 菜单权限树按业务菜单逐项勾选：勾选菜单默认携带该菜单下全部按钮与接口子资源；“诊断工作台-诊断任务”菜单同时包含全部诊断任务节点按钮。
...
- 业务资源树：酒店管理、酒店管理-诊断工作台、酒店管理-诊断任务、酒店管理-诊断日志四个业务菜单，各自挂载按钮与接口资源；“酒店管理-诊断任务”菜单下的按钮资源包含全部诊断任务节点。系统管理菜单挂载用户管理、角色管理、菜单管理三个子菜单及各自接口。
...
- 修改密码区校验规则与用户管理一致，另附加两条：新密码不得与前三个旧密码相同；修改成功提示“修改成功，请重新登录”并强制退出登录，需使用新密码重新登录。
...
- 运营人员只能看到管理员分配的酒店。
- 新增账户后按其绑定角色的资源并集生效；内置超级管理员角色拥有全部菜单、按钮、接口资源，任务权限默认全部开通。
- 运营人员的酒店管理页仅展示并诊断被配置的酒店，未配置酒店不可进入。
- 运营人员未开通的菜单页面不可进入；“系统管理”分组对运营人员不展示。
- 任务权限被关闭的节点：任务详情展示权限关闭提示，提交与忽略操作被拦截。
- 运营人员将任务处理为“以后不再提醒”后，其该任务节点权限自动关闭；管理员在用户管理重新开启后可继续处理。
- 管理员可见“系统管理”分组并可进入用户管理、角色管理、菜单管理；“我的信息”对全部账户在左侧导航一级菜单展示。
- 平台不提供切换账号功能；新增账户仅管理员可在用户管理中操作。
- 点击“新增酒店”输入酒店名称即可新增；新增酒店抖音来客与携程官网均展示未授权，授权列展示对应平台状态，“诊断”按钮置灰。
...
- 权限判定链：按账户的角色并集取得资源集合，前端按菜单资源渲染导航、按按钮资源控制操作显隐，后端按接口资源校验请求合法性。
- 诊断任务节点作为“酒店管理-诊断任务”菜单下的按钮级资源参与判定；管理员账户不受资源开关限制。

### 17.2 用户管理

- 用户管理为一级页面，位于“系统管理”分组，仅管理员可访问。
- 账户字段：账户名（登录凭证，创建后不可修改）、用户名称（展示名）、角色（多选）、账户状态（启用 / 禁用）、创建时间、更新时间。
- 新增账户时设置初始密码；密码 8-16 位，必须同时包含数字、大写字母、小写字母和特殊符号，禁止空格；编辑账户不回显密码，重置密码为独立操作。
- 支持编辑、禁用、删除和重置密码；删除与重置密码均需二次确认；内置管理员账户不可禁用、不可删除。
- 管理员支持对每一个账户配置酒店；配置后的酒店在该账户登录后的酒店管理页面即可查看和诊断，未配置的酒店不出现在其酒店管理页面。
- 管理员支持按诊断任务节点为账户恢复被关闭的任务权限。

### 17.3 角色管理

- 角色管理为一级页面，位于“系统管理”分组，仅管理员可访问。
- 角色字段：角色名称（必填、唯一）、状态、备注、菜单权限树。
- 菜单权限树按业务菜单勾选，勾选菜单默认携带该菜单下全部按钮与接口子资源；“酒店管理-诊断任务”菜单包含全部诊断任务节点按钮。
...
- 修改密码遵循统一密码策略：8-16 位、四类字符齐全、禁止空格、二次输入一致；新密码不得与前三个旧密码相同。
- 修改成功后强制重新登录，全部会话清空。

### 17.6 任务权限与任务处理联动

- 账户处理任务时，如将任务处理为“以后不再提醒”，则该账户默认被关闭该任务节点的任务权限。
- 权限被关闭后，该账户不可再处理该节点的任务；任务详情展示权限已关闭提示，提交与忽略操作被拦截。
- 如后续仍需处理该任务，需要让管理员在用户管理的“任务权限”配置里打开该权限（菜单管理中开启对应按钮资源可完成同等操作）。
