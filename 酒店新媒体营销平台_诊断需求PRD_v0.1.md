...
- 门店头图补充方案按抖音来客「门店头图」表单呈现：头图形式（视频 / 图片）单选，默认图片；图片预选自携程「精选图」（标注所选来源与图片名），可更换；同时展示对应形式的规格与上传规范。
- 商家相册按缺图类别逐卡片展示：相册类别、当前张数、建议上传张数（可修改）、已选图片（预选自携程对应类别，可更换）；携程与抖音类别一一对应映射。
- 图片、张数与头图形式均可修改；提交后 AI 仅将用户确认的图片写入抖音来客，写入后重新读取并校验。
...
- 新增方案以抖音来客「创建预售房型」表单呈现，全部内容可修改：
  ...
  - 预售房型名称：物理房型名称（系统生成）＋ 自定义部分（必填，40 字以内，可修改）。
