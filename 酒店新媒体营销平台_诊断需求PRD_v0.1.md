...
- 检查项状态为六种：采集失败（抖音来客）、采集失败（携程）、采集失败（抖音来客、携程）、采集成功＋通过、采集成功＋不通过；抖音来客与携程均采集成功时按规则展示“采集成功＋通过”或“采集成功＋不通过”。状态涉及抖音来客采集失败时，右侧提示“请前往抖音来客检查此项是否符合标准”；仅携程采集失败且所采集数据非价格数据的，右侧提供“重试”按钮，点击后对该检查项重新发起携程采集（转圈样式状态），最多重试 5 次，超过 5 次后按钮置灰并提示“采集异常，请联系技术管理员”，重试成功后按采集结果更新为通过或不通过。状态以彩色底色标签醒目展示：采集成功为绿底白字，通过为浅绿底绿字，不通过为红底白字，采集失败为橙底白字；抖音来客采集失败提示换行展示在状态下方，以浅橙底色突出显示。
...
| 抖音无对应字段 | 不诊断、不展示、不写入 |
| 页面加载或采集失败 | 展示对应“采集失败”状态；涉及抖音来客的提示“请前往抖音来客检查此项是否符合标准”；仅携程且非价格数据的提供“重试”（最多 5 次，超过后置灰并提示“采集异常，请联系技术管理员”）；不把节点判为通过 |
...
- 检查项状态为“采集失败（抖音来客）”“采集失败（携程）”“采集失败（抖音来客、携程）”“采集成功＋通过”“采集成功＋不通过”；采集失败涉及抖音来客的，状态右侧提示“请前往抖音来客检查此项是否符合标准”；仅携程采集失败且非价格数据的，状态右侧提供“重试”按钮（转圈样式重新采集，最多 5 次，超过后按钮置灰并提示“采集异常，请联系技术管理员”）。状态以彩色底色标签醒目展示（采集成功绿底白字、通过浅绿底绿字、不通过红底白字、采集失败橙底白字），抖音来客采集失败提示换行展示在状态下方并突出显示。该步骤生成的任务在步骤备注面板下方“本步骤已生成任务”区逐行展示，可在诊断任务列表中进入该任务详情。
...
- 检查项覆盖六种状态展示，状态以彩色底色标签醒目展示（采集成功绿底白字、不通过红底白字、采集失败橙底白字），抖音来客采集失败提示换行并突出显示；抖音来客采集失败含“请前往抖音来客检查此项是否符合标准”提示，携程采集失败（非价格数据）支持重试且最多 5 次，超过后按钮置灰并提示“采集异常，请联系技术管理员”。
