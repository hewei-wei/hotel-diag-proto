...
- 诊断记录默认收起，当次（最新一次）诊断默认展开；点击行展开或收起该次诊断的任务明细，逐条展示任务名称、任务路径、任务状态（执行中、执行成功、执行失败、已忽略）和“详情”按钮；执行失败的任务展示失败原因；待处理任务不在已处理展示。
...
- 生成任务数为该次诊断生成的任务总数；执行中数量、执行成功数、执行失败数、已忽略数为该次诊断各执行状态的任务数；待处理任务不计入已处理展示。
- 展开某次诊断后逐条展示任务明细：任务名称、检查路径、任务状态（执行中、执行成功、执行失败、已忽略）和“详情”按钮；执行失败的任务展示失败原因。
- “详情”只读展示该任务处理时的数据快照。
...
- 记录默认收起，最新一次诊断默认展开；展开后逐条展示该次诊断的任务名称、任务路径、任务状态（执行中、执行成功、执行失败、已忽略）和“详情”按钮，执行失败任务展示失败原因；“详情”只读展示该任务处理时的数据快照。
...
- 已处理诊断记录按次展示任务ID、诊断名称、生成任务数、执行中数量、执行成功数、执行失败数、已忽略数和诊断时间，默认收起、最新一次默认展开，展开后逐条展示任务明细与失败原因，“详情”只读展示处理数据快照。
- 任务不展示抖音不存在的字段。
- 携程无值时 AI 不自行补写。
- 任务、忽略状态和执行结果按酒店隔离。
...
- 已处理按诊断次数分组展示历史诊断记录（任务ID、诊断名称、生成任务数、执行中数量、执行成功数、执行失败数、已忽略数、诊断时间），展开后逐条展示任务名称、任务路径、任务状态（执行中、执行成功、执行失败、已忽略）和“详情”（只读数据快照），执行失败任务展示失败原因。
