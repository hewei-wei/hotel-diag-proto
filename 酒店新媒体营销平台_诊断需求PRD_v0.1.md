...
- 新增酒店。
- 将酒店分配给运营人员。
- 在权限管理中维护通用权限（菜单权限、按钮权限、任务权限），并可编辑所有权限名称。
- 新增子账号（运营人员账号），支持新增多个。
- 为每个子账号配置酒店，配置后的酒店在子账号登录后的酒店管理页面即可查看和诊断。
- 在账号管理中恢复子账号被关闭的任务权限。
...
- 将任务处理为“以后不再提醒”后，该任务节点的任务权限自动关闭；如后续仍需处理，由管理员在账号管理中重新开启。
...
- 线下任务的完成记录。
- 诊断日志。
- 账号管理：管理员新增子账号并为子账号配置酒店。
- 权限管理：通用权限配置（菜单权限、按钮权限、任务权限），权限名称可编辑。
...
账号管理与权限管理位于“系统管理”分组，与酒店管理平级，仅管理员可见、可进入；运营人员的左侧导航不展示该分组。


酒店管理页承载逐酒店的抖音来客与携程官网授权入口。左侧导航固定提供“酒店管理”菜单，管理员账号另展示“系统管理”分组（含“账号管理”“权限管理”）；运营人员点击“诊断”进入对应酒店的二级诊断页面（诊断工作台），全部诊断在诊断工作台内按步骤完成。进入具体酒店后，右侧内容区顶部展示二级导航：返回酒店管理入口、当前酒店名称和“诊断工作台”“诊断任务”“诊断日志”页签，酒店相关页面通过二级导航切换。
...
| 左侧导航 | 系统管理 | 展示“系统管理”分组与“账号管理”“权限管理”入口，仅管理员账号展示该分组 |
...
| 页面头部 | 标题 | 展示“酒店管理” |
| 页面头部 | 新增酒店按钮 | 页面头部右侧展示“新增酒店”按钮；点击弹出新增弹窗，输入酒店名称确认后新增酒店，抖音来客和携程官网默认未授权；按钮受权限管理中的按钮权限控制 |
...
酒店管理页页面头部提供“新增酒店”按钮：点击后弹窗输入酒店名称即可新增；新增酒店默认抖音来客和携程官网均为未授权，需完成授权后才可诊断。“新增酒店”按钮受权限管理中的按钮权限控制。
...
- 以后不再提醒：点击后弹出二次确认弹窗，确认后关闭本次任务并转为已处理，任务状态为“已忽略”；同时关闭该酒店该节点后续自动诊断提醒，直至主动恢复。运营人员确认时，其该任务节点的任务权限同步关闭，后续同节点任务不可再处理，需管理员在账号管理或权限管理中重新开启；管理员确认时不涉及任务权限变化。
...
- 入口位于左侧导航“系统管理”分组，仅管理员可进入；运营人员左侧导航不展示该分组。
- 页面头部提供“新增子账号”按钮，弹出新增弹窗：填写员工姓名和登录手机号；确认后创建子账号，默认继承权限管理中的通用权限配置（菜单权限、按钮权限、任务权限全部开通）。
- 子账号列表逐行展示子账号姓名、手机号、配置酒店和任务权限状态（“全部开启”或“N 项已关闭”）。
- 每行提供“配置酒店”和“任务权限”操作。
- “配置酒店”弹窗逐酒店勾选，展示酒店名称和门店副名称；保存后配置的酒店出现在子账号登录后的酒店管理页面，可查看和诊断。
- “任务权限”弹窗按诊断任务节点平铺展示，逐节点开关；用于将子账号被自动关闭的任务权限重新开启。
- 子账号将任务处理为“以后不再提醒”后，该任务节点权限自动关闭，账号管理列表的任务权限状态同步更新。

### 15.6 权限管理页

- 入口位于左侧导航“系统管理”分组，与账号管理平级，仅管理员可进入。
- 权限为通用配置，对所有子账号生效，页面不展示子账号名称。
- 权限分三块：菜单权限、按钮权限、任务权限；所有权限名称均可编辑（点击权限名进入编辑，回车或失焦保存）。
- 菜单权限区按菜单页面逐项开关，包括：酒店管理、酒店管理-诊断工作台、酒店管理-诊断任务、酒店管理-诊断日志。
- 按钮权限区包含酒店管理页及二级页面（诊断工作台、诊断任务、任务详情、诊断日志）的所有按钮，逐项开关；其中“新增酒店”按钮权限默认置于按钮权限中。
- 任务权限区以抖音来客的诊断路径分组展示诊断任务节点，逐节点开关。
- 管理员自身不受权限限制。
...
- 运营人员只能看到管理员分配的酒店。
- 新增子账号后默认继承权限管理中的通用权限配置；通用权限全部开通时即拥有全部菜单权限，任务权限默认全部开通。
- 运营人员的酒店管理页仅展示并诊断被配置的酒店，未配置酒店不可进入。
- 运营人员未开通的菜单页面不可进入；“系统管理”分组对运营人员不展示。
- 任务权限被关闭的节点：任务详情展示权限关闭提示，提交与忽略操作被拦截。
- 运营人员将任务处理为“以后不再提醒”后，其该任务节点权限自动关闭；管理员在账号管理重新开启后可继续处理。
- 管理员可见“系统管理”分组并可进入账号管理、权限管理。
- 点击“新增酒店”输入酒店名称即可新增；新增酒店抖音来客与携程官网均展示未授权，授权列展示对应平台状态，“诊断”按钮置灰。
- 权限管理为通用配置，页面不展示子账号名称；菜单权限、按钮权限、任务权限名称均可编辑，编辑后各处展示同步更新。
...
- 账号管理对管理员名下的普通员工（子账号）进行管理，管理员支持新增多个子账号。
- 新增子账号时填写员工姓名和登录手机号；创建后默认给子账号分配所有的菜单权限。
- 管理员支持对每一个子账号配置酒店；配置后的酒店在子账号登录后的酒店管理页面即可查看和诊断，未配置的酒店不出现在子账号的酒店管理页面。
- 账号管理列表展示每个子账号的手机号、配置酒店和任务权限状态。

### 17.2 权限管理

- 权限管理为一级页面，与账号管理平级，位于“系统管理”分组，仅管理员可访问。
- 权限为通用配置，对所有子账号生效，页面不展示子账号名称。
- 权限管理含三块权限，所有权限名称均允许编辑：
  1. 菜单权限：即每一个菜单页面的权限，包括酒店管理、酒店管理-诊断工作台、酒店管理-诊断任务、酒店管理-诊断日志。
  2. 按钮权限：即酒店管理页及二级页面（诊断工作台、诊断任务、任务详情、诊断日志）所有按钮的权限；“新增酒店”按钮权限默认置于其中。
  3. 任务权限：即每一个诊断任务的节点权限，以抖音来客的诊断路径分组展示。
- 权限名称编辑后，菜单导航、页面按钮和任务权限展示同步使用新名称。
- 管理员自身不受权限限制。

### 17.3 任务权限与任务处理联动

- 子账号处理任务时，如将任务处理为“以后不再提醒”，则该子账号默认被关闭该任务节点的任务权限。
- 权限被关闭后，子账号不可再处理该节点的任务；任务详情展示权限已关闭提示，提交与忽略操作被拦截。
- 如后续仍需处理该任务，需要让管理员在账号管理中相应子账号的任务权限配置里打开该权限（权限管理页可完成同等操作）。
