...
用户管理、角色管理、菜单管理位于“系统管理”分组，与酒店管理平级，仅管理员可见、可进入；“我的信息”为左侧导航一级菜单，全部账户可见、可进入。运营账户的左侧导航不展示系统管理分组。平台不提供切换账号功能：登录账户由管理员在用户管理中统一创建，右上角账号区仅展示当前登录账户的名称与角色。
...
| 顶部栏 | 账号区 | 位于页面右上角，展示当前账号头像、账号名称和角色；仅作展示，不提供切换账号入口 |
| 二级导航 | 酒店二级页签 | 进入具体酒店后，在顶部栏下方展示：返回酒店管理入口、当前酒店名称和门店副名称，以及“诊断工作台”“诊断任务”“诊断日志”页签；一个页签对应一个页面，当前页签高亮，“诊断任务”页签展示待处理任务数角标 |
| 页面头部 | 标题 | 展示“酒店管理” |
| 页面头部 | 新增酒店按钮 | 页面头部右侧展示“新增酒店”按钮；点击弹出新增弹窗，输入酒店名称确认后新增酒店，抖音来客和携程官网默认未授权；按钮受菜单管理中对应按钮资源的权限控制 |
...
- 页面由查询条件区和数据列表组成：查询条件含“账户名 / 用户名称”关键词与“账户状态”筛选，提供“查询”“重置”按钮；列表标题为“数据列表”，底部展示“共 X 条”与分页。
- 列表列依次为：账户名、用户名称、角色、账户状态（开关）、创建时间、更新时间、操作。
- 页面头部提供“新增账户”按钮，弹窗字段为：账户名、密码、确认密码、用户名称、角色（多选）、账户状态。校验规则：
  - 账户名必填且禁止空格，为登录凭证，创建后不可修改。
  - 密码由 8-16 位数字、大小写字母组成，不能包含空格；确认密码必须与密码一致。
  - 用户名称为展示名，与账户名分离，必填。
  - 角色至少选择一个，可选范围不含内置超级管理员角色。
- 行操作提供：编辑（用户名称、角色、账户状态；密码不回显）、配置酒店、重置密码、删除。
...
- 账户字段：账户名（登录凭证，创建后不可修改）、用户名称（展示名）、角色（多选）、账户状态（启用 / 禁用）、创建时间、更新时间。
...
- “我的信息”为左侧导航一级菜单，全部账户可进入，只读查看账户名、用户名称、角色和账户状态。
...
- 任务详情集中展示最低价、第二低价格、最高价三个档位及档位内房型实时价格与各档诊断结果，标明本次补充的档位房型。
