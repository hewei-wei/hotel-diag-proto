...
酒店管理页承载逐酒店的抖音来客与携程官网授权入口。左侧导航固定提供“酒店管理”菜单和“我的信息”一级菜单，管理员账号另展示“系统管理”分组（含“用户管理”“角色管理”“菜单管理”）；运营人员点击“诊断”进入对应酒店的二级诊断页面（诊断工作台），全部诊断在诊断工作台内按步骤完成。进入具体酒店后，右侧内容区顶部展示二级导航：返回酒店管理入口、当前酒店名称和“诊断工作台”“诊断任务”“诊断日志”页签，酒店相关页面通过二级导航切换。
...
任务处理状态仅针对单次诊断生成的任务，分为“待处理”“已处理”两级；“已处理”任务同时展示执行状态：执行中、执行完成、执行失败、已忽略（此次）。
...
- 已处理列表字段：任务名称、任务路径、任务提交时间、任务状态（执行中、执行完成、执行失败、已忽略（此次））。
- 已处理页签提供任务状态筛选（全部、执行中、执行完成、执行失败、已忽略（此次），各选项展示对应任务数）和任务名称搜索框；筛选与搜索共同生效，无匹配任务时展示空态提示。
...
- 表格字段为：诊断次数、任务数、执行成功数、执行失败数、详情。
- 诊断次数列展示“第 N 次诊断”与该次诊断时间。
...
- 页面主体为一张日志表格，每次诊断一行，按诊断时间倒序排列，表头字段为：诊断次数、任务数、执行成功数、执行失败数、详情。
- 诊断次数列展示“第 N 次诊断”与诊断时间；任务数、执行成功数、执行失败数为该次诊断生成任务的执行结果统计。
...
- 资源权限树与菜单管理资源树结构一致，覆盖酒店管理、酒店管理-诊断工作台、酒店管理-诊断任务、酒店管理-诊断日志四个业务菜单，按菜单、按钮、接口逐项勾选：勾选菜单默认同时勾选其下全部按钮与接口子资源，子资源可单独取消或补充，不支持按菜单整体授权。系统管理资源仅管理员使用，不参与角色授权。
- 内置超级管理员角色展示“内置”标签，不可编辑、不可停用、不可删除，拥有全部资源。
- 角色被账户引用时不可删除，并提示已关联账户；未被引用的角色删除前弹出二次确认。
- 停用角色后不可再被新账户选择；已绑定该角色的账户不受影响。
- 示例角色覆盖四类权限形态：超级管理员（内置，全部资源）、运营管理员（四个业务菜单及全部按钮与接口，覆盖酒店接入、平台授权与诊断全流程）、酒店店长（酒店管理、诊断工作台、诊断任务三个菜单，不含新增酒店与诊断日志）、质检专员（诊断任务、诊断日志两个菜单与日志详情按钮，只读查看、不可处理任务）。
...
- 业务资源树：酒店管理、酒店管理-诊断工作台、酒店管理-诊断任务、酒店管理-诊断日志四个业务菜单，各自挂载按钮与接口资源。系统管理菜单挂载用户管理、角色管理、菜单管理三个子菜单及各自接口。
...
- 诊断日志以表格按次展示每次诊断的任务数、执行成功数与执行失败数，点击详情展开该次诊断的逐条任务明细。
- 任务不展示抖音不存在的字段。
- 携程无值时 AI 不自行补写。
...
- 点击“此次忽略”弹出二次确认弹窗，确认后任务转为已处理，任务状态为“已忽略（此次）”；下次诊断仍检查该节点。
- 已处理列表展示任务名称、任务路径、任务提交时间和任务状态（执行中、执行完成、执行失败、已忽略（此次））。
...
- AI 不生成或发布评价，不修改抖音字段。
- 任务详情操作为“已完成”“此次忽略”：用户线下执行后点击“已完成”，任务转为已处理（状态：执行完成）；不提供“提交”按钮。
