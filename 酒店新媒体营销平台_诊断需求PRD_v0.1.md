...
3. 最新累计规则表 `outputs/activity_square_rule_20260827/抖音来客诊断规则表.xlsx` 中的“诊断规则”工作表；其已确认内容已完整转写至本文档第 19 章。
...
- Excel 保留为规则维护底稿；本文档第 19 章已将当前 26 条规则完整转写，阅读和评审需求时无需再打开 Excel。
...
1. 先选酒店，再诊断或执行。
2. 酒店数据、授权、诊断、任务、忽略配置、执行记录和日志按 `hotel_id` 隔离。
...
- 新增酒店。
- 将酒店分配给运营人员。
- 在活动广场的管理员配置中，查看活动数据并打开或关闭活动的用户端诊断资格。
...
- 诊断节点、字段映射和策略以本文档第 19 章为准；新增规则需先完成需求确认并同步本文档。
...
酒店管理页承载逐酒店的抖音来客与携程官网授权入口。左侧导航固定提供“酒店管理”菜单和“系统管理”分组；运营人员点击“诊断”进入对应酒店的二级诊断页面（诊断工作台），全部诊断在诊断工作台内按步骤完成。进入具体酒店后，右侧内容区顶部展示二级导航：返回酒店管理入口、当前酒店名称和“诊断工作台”“诊断任务”“执行记录”“活动配置”页签，酒店相关页面通过二级导航切换。

### 5.2 诊断业务菜单

诊断路径以抖音来客菜单结构和本文档第 19 章为准：
...
酒店管理页由全局框架、页面头部、前置条件提示、检索筛选和酒店列表五部分组成。左侧导航在各页面保持同一结构：品牌区、“酒店管理”菜单和“系统管理”分组；进入具体酒店后，酒店相关页面（诊断工作台、诊断任务、执行记录、活动配置）通过右侧内容区顶部的二级导航切换，二级导航持续展示当前酒店名称，并提供返回酒店管理页的入口。酒店管理页全部元素如下：
...
| 二级导航 | 酒店二级页签 | 进入具体酒店后，在顶部栏下方展示：返回酒店管理入口、当前酒店名称和门店副名称，以及“诊断工作台”“诊断任务”“执行记录”“活动配置”页签；一个页签对应一个页面，当前页签高亮，“诊断任务”页签展示待处理任务数角标 |
| 页面头部 | 标题 | 展示“酒店管理” |
...
- 数据不足：按本文档第 19 章对应节点规则处理；不得自行推测。
...
- 步骤备注面板在检查项列表下方提供“本步骤已生成任务”区，任务按行独立展示：每行展示任务名称和对应检查项，行右侧提供“去处理”按钮，点击直接进入该任务的详情页处理；联合诊断节点多项不通过合并为一个任务，在该区展示为一条。
...
如节点存在“暂无此设施”、图片补充、字段编辑或多个独立确认方案，以本文档第 19 章对应节点的任务交互为准。

### 10.4 任务展示原则

- 展示当前酒店。
- 展示诊断节点和问题说明。
- “AI 诊断建议”文案只描述检查结论、数据差异和用户需确认的操作，不描述数据采集过程与任务生成过程。
- 展示规则规定的数据来源和证据。
- 展示抖音当前值、携程可映射值及差异。
- 展示将要写入抖音的字段和用户可编辑内容。
- 不展示抖音来客不存在的字段。
- 任务详情与所选任务一一对应，展示该任务的诊断建议、来源数据与差异和建议写入字段；找店指引任务详情按第 19.2.5 节先选择指引方式，再按路线节点或指引视频填写；中差评回复任务按第 19.12.1 节逐条确认回复内容；活动报名任务按第 19.13.1 节逐个确认报名意向。
- 用户未确认前不得执行。
...
下列目录来自最新累计 Excel 规则表。每个节点的完整诊断条件、字段映射、策略、任务粒度、确认动作、所需数据和数据来源详见本文档第 19 章。
...
| 22 | 经营成长 | 商家经营分 | 装修 | 装修评分 |
| 23 | 经营成长 | 商家经营分 | 商品 | 商品评分 |
| 24 | 经营成长 | 商家经营分 | 评价 | 评价评分 |
| 25 | 经营成长 | 评价管理 | 评价回复 | 待回复中差评 |
| 26 | 带货与推广 | 活动广场 | 活动管理 | 活动参与与配置 |

## 13. 活动广场管理要求

### 13.1 数据采集

- 每次诊断采集当前活动的唯一标识、名称、状态、详情及采集时间。
- “参与详情”表示已参与，“立即报名”表示未参与。

### 13.2 管理员配置

- 新活动默认关闭。
- 管理员打开后，该活动才可进入用户端诊断及报名候选。
- 管理员关闭不修改抖音活动状态。
- 历史活动本次未出现时不删除，标记为已下线。
- 活动重新上线时保留原管理员开关。

### 13.3 用户端任务

- 仅展示“在线＋管理员打开＋立即报名”的活动。
- 一个活动一条任务，分别确认、提交、执行和校验。
- 用户确认后，AI 才执行报名。
- 用户可选择此次忽略。
...
- 执行记录以“检查路径 → 诊断次数”两级结构组织。
- 记录按检查路径分类归组，同一路径的全部历史诊断归入同一组。
- 分组头部展示该路径的检查路径、已诊断次数和最近一次诊断结果与时间。
- 展开分组后按诊断次数展示明细：每次诊断展示诊断次序、诊断时间、诊断结果（通过或不通过）和本次生成任务数，按时间倒序排列。
...
- 进入具体酒店后，右侧内容区顶部展示二级导航：返回酒店管理入口、当前酒店名称和门店副名称，以及“诊断工作台”“诊断任务”“执行记录”“活动配置”页签；“诊断任务”页签展示待处理任务数角标。
- 授权入口位于对应酒店行。
...
- 焦点区下方左侧为步骤列表，按检查路径展示全部步骤，示例步骤为“基本信息、酒店政策、酒店设施、门店装修、房型管理、日历房、房价房态管理、预售房型、团购、装修、商品、评价、评价回复、活动管理”，覆盖店铺管理、商品与货架、经营成长、带货与推广四条一级路径；每个步骤以“未诊断”“诊断中”“诊断完成”状态标签展示。
- 步骤按顺序逐个执行，当前步骤完成后自动进入下一步骤；执行中的步骤自动滚动到可视区域。
- 点击步骤，右侧“步骤备注”面板展示该步骤的检查项、检查规则和逐项结果；诊断执行时备注跟随当前步骤。
- 检查项结果为“通过”或“不通过”；该步骤生成的任务在步骤备注面板下方“本步骤已生成任务”区逐行展示，任务行右侧“去处理”直接进入该任务详情。
...
- 执行记录按检查路径分组展示，分组头部展示检查路径、已诊断次数、最近一次诊断结果和时间。
- 展开分组后按诊断次数展示每次诊断的次序、时间、结果和生成任务数。
...
- 检查项通过时不生成任务；检查项不通过时生成诊断任务，任务在步骤备注“本步骤已生成任务”区逐行展示，任务行“去处理”进入该任务详情。
...
- 执行记录按检查路径分组、按诊断次数展示每次诊断结果和生成任务数。
- 任务不展示抖音不存在的字段。
- 携程无值时 AI 不自行补写。
...
- 中差评回复任务逐条展示待回复评价，每条评价提供三套答复语供单选，用户可修改选中内容后提交。
- 活动报名任务逐个展示可报名且本酒店未参加的活动，每个活动通过下拉单选报名意向（参加、不参加），默认选中“参加”，可修改后一次提交。
...
| 目标字段 | 所需数据 | 处理规则 |
|---|---|---|
| 抖音门店名称 | 酒店名称、酒店位置 | 基于酒店方圆 3 公里热门地标获取两个地标简称，并按公式生成 2 个候选名称 |

AI 检索提示口径为：“XX酒店方圆3公里中最热门的两个地标建筑，直接给我两个地点的简称。”AI 只生成 2 个候选名称。

**任务与执行**

- 任务：门店名称修改建议任务。
- 确认修改：用户选择候选名称，AI 写入并校验。
- 自行编辑后修改：用户编辑名称，AI 按用户文本写入并校验。
- 此次忽略：关闭本次任务，下次继续诊断。
...
- 任务页分五张卡片展示携程原始政策、建议值和可编辑抖音字段。
- 任一分组必填项不完整时禁止提交。
- 提交：AI 统一填写并提交酒店政策，全部分组校验成功后完成。
- 此次忽略、以后不再提醒均作用于整个联合节点，不拆分到单一政策。
- 数据来源：携程 PC 端；入住方式由用户选择。

【待补充】携程缺少某一政策时如何完成五组必填；早餐“每日开放/指定时间”的判断条件；餐食形式、类型、菜品与抖音选项的完整字典；“携程 PC 端”是否统一为携程用户端。

---

### 19.4 店铺管理 / 酒店信息管理 / 酒店设施
...
4. 携程未查到设施：仍创建设施确认任务，展示“未查到对应数据”和抖音当前值；当前为无时默认无。

**任务展示与执行**

- 以抖音字段为边界，展示携程数据、抖音当前值、建议值和可编辑控件。
- 用户保持“无”：点击“暂无此设施”，关闭本次任务，不修改抖音、不进入执行中。
- 用户改为“有”：若抖音存在额外/必填字段则展开；仅写入用户确认内容。
- 携程有数据：用户确认后提交，AI 写入并校验。
- 仅有有/无开关的设施，切换为有后不展示额外字段。
- 任务按 `hotel_id + facility_code` 独立。
...
- 抖音酒店设施顶部不存在“审核不通过”标签时通过；存在时生成独立设施审核处理任务。
...
- 提交审核：AI 使用用户新图片提交审核并校验。
- 暂无此设施：AI 将设施由有改为无并校验。
- 此次忽略：本次关闭，下次仍检查审核标签。
- 以后不再提醒：只关闭审核不通过标签诊断，不影响设施常规诊断。

【规则冲突待确认】Excel 将所需数据写为携程“服务及设施”，但本任务实际诊断证据来自抖音审核状态、原因和被驳回图片；是否还需要展示携程设施数据需确认。
...
**诊断规则**：读取抖音审核标识；无驳回通过，有驳回则进入“去设置”，读取失败原因、节点和被驳回图片。

**任务展示**：审核节点、失败原因、当前驳回图、携程分类图片、用户选择/上传入口。

**执行动作**

- 手动替换并提交：用户选择图片后，AI 写入并提交审核。
- 确认删除：AI 删除驳回图片并校验。
- 此次忽略：下次仍诊断。
...
- 获取抖音当前外显小图，与携程素材池“外观”类别逐图比对。
- 查到相同图则通过；没有相同图时展示抖音当前图和携程外观图。
- 用户选择携程外观图并提交后，AI 替换并校验。
- 支持此次忽略和以后不再提醒；长期关闭只作用于当前酒店外显小图。

【待补充】图片一致性判定采用原图链接、文件哈希、感知相似度还是人工确认；相似度阈值未定义。
...
- 用户选择图片后提交，AI 只上传用户确认图片并校验。
- 支持此次忽略、以后不再提醒。

【待补充】“携程图片类别→抖音商家相册类别”的完整一一对应表；门店头图/相册的数量、格式、尺寸、大小和审核要求。
...
1. 名称一致自动对应；名称不同但可可靠匹配时展示建议关系供用户确认；不能可靠匹配时必须先由用户选择对应抖音房型或“没有对应房型”。
2. 对应关系未确认前不得修改或新增。
3. 不能只比较房型数量。抖音多出房型不删除；携程每个房型都需找到对应项并校验。
...
- 已有房型差异任务：展示对应关系、每个差异字段的携程值、抖音值、建议值和可编辑控件。
- 缺失房型新增任务：展示全部字段，用户必须确认并填写该房型总房间数；AI 选择抖音“手动创建”。
...
【待补充】楼层、面积、床型、窗户、人数、网络服务在抖音房型描述中的精确控件、格式、单位、枚举和值转换；房型设施完整映射字典；房型图片数量和规格；“可靠匹配”的判断标准。
...
任务展示房型名称、携程早餐份数、抖音当前份数和建议份数；提交后仅修改早餐份数。
...
4. 两种都缺少：同一预售房型补充任务内展示两个可独立确认方案；只缺一种则只展示缺失方案。
...
| 商品搭配—餐食 | 关联物理房型有免费早餐时，添加早餐及份数 | 同左 | 携程用户端 |
| 商品搭配—礼遇 | 已核实免费特殊服务及份数 | 同左 | 携程用户端 |
| 包含数量 | 基础方案 1 夜 | 基础方案 1 夜 | 按实际方案间夜数 |
| 划线价 | 携程对应物理房型划线价 | 同左 | 携程用户端 |
| 售价 | 日常每晚价×间夜数×95% | 最近节假日期间最低每晚价×间夜数×95% | 携程用户端 |
| 套餐图 | 对应物理房型图片 | 同左 | 携程用户端 |
| 不可入住日期 | 选择节假日不可用，并使用抖音提供的时间区间 | 不选择节假日不可用 | 抖音页面选项 |
...
- 补充任务按预售房型生成；同一房型可在一条任务内独立勾选日常/节假日方案。
- 已有商品价格问题：一个商品一条价格任务。
- 已有商品名称问题：一个商品一条名称任务。
- 同一商品价格与名称问题必须拆分，用户可只处理其中一个。
- AI 新增前再次检查目标商品，修改前再次读取当前值，完成后复核。
- “以后不再提醒”按当前对象和诊断分支关闭；价格关闭不影响名称，名称关闭不影响价格。
...
商家经营分是独立诊断入口，复用对应业务菜单的字段映射和执行规则。评分节点满分时通过；未满分时进入对应业务菜单处理。业务菜单路径与商家经营分路径的长期关闭配置相互独立：业务菜单路径的“以后不再提醒”不阻止商家经营分路径生成任务，商家经营分路径的处理也不改变业务菜单路径的长期关闭配置。

#### 19.11.1 装修评分

| 评分节点 | 通过条件 | 处理路径与策略 |
|---|---|---|
| 配置开业时间和装修时间 | 对应评分满分 | 进入门店管理，复用开业/装修时间规则 |
| 有图服务设施及热门设施 | 热门设施≥6；带图设施≥3 | 进入酒店设施，展示当前数量、尚缺数量、全部无状态设施、携程图片和上传区 |
| 门店营业状态 | 正常营业 | 复用营业状态规则 |
| 联系方式 | 不为空 | 复用联系电话规则 |
| 相册高清图片 | 至少 16 张 | 复用头图和相册规则，优先携程分类图片，不足由用户上传 |

热门设施不足时，用户至少选择尚缺数量的真实设施由无改为有；AI 执行后以抖音重新读取的实际认定数量为准。带图设施图片不得由 AI 生成。未达到目标数量不得完成。

【关键待补充】当前 26 条规则中没有“开业时间”和“装修时间”的原始诊断、字段映射、任务和执行规则，现阶段不能直接复用，需优先补齐。
...
| 创建商品 | 复用房型管理规则 |
| 创建客房＋其他商品 | 复用房型管理规则 |
| 预留间夜，未来 30 天有房可订 | 复用日历房规则 |
| 开通日历房商品 | 复用日历房规则 |

商家经营分入口不改变原任务粒度，仍按房型分别确认和执行。

【待补充】“创建商品”和“创建客房＋其他商品”均复用房型管理是否足以使经营分满分，需结合抖音实际评分要求验证。

#### 19.11.3 评价评分

- 评价下只有一个评分节点；满分通过，未满分表示好评不足。
- 形成一条线下“评价经营建议任务”。
- 统一建议：“可通过提升服务、赠送正常入住礼遇或提供合理的房型升级改善住客体验，并邀请住客自愿发表真实评价。”
- 目标为每日争取至少 5 条真实住客抖音好评，主要面向通过抖音购买并实际入住的客人。
- AI 不生成或发布评价，不修改抖音字段。
- 用户线下执行后点击完成，也可此次忽略。
...
| 活动唯一标识 | 优先用于识别同一活动 |
| 活动名称 | 可更新业务字段 |
| 活动状态 | “参与详情”=已参与；“立即报名”=未参与 |
| 活动详情 | 当前页面详情 |
| 在线/下线状态 | 本次未出现的历史活动标记下线 |
| 首次/最近采集时间 | 用于同步记录 |
| 管理员开关 | 打开/关闭，数据同步时保留 |
...
- 新活动首次进入后台时默认关闭。
- 打开：允许进入用户端诊断和报名候选。
- 关闭：不展示、不诊断、不生成任务；不改变抖音活动状态。
- 同一活动业务字段更新时以最新采集值覆盖当前已存业务字段，并保留管理员开关。
- 历史活动本次未出现时不删除，标记已下线并保留开关；重新出现时恢复在线并保留原开关。

**用户端规则**

- 只展示“在线＋管理员打开＋立即报名”的活动；“参与详情”视为已参与，不进入任务。
- 诊断生成一个“活动报名”任务；任务详情逐个展示可报名且本酒店未参加的活动（活动名称、活动说明、活动时间）。
- 每个活动右侧提供报名意向下拉单选（参加、不参加），默认选中“参加”，可修改。
- 报名意向确认后一次提交。
- 没有符合条件的活动时节点通过。
...
- 提交前再次检查在线和参与状态。
- 报名意向为“参加”的活动：用户确认后 AI 点击立即报名并完成当前报名流程，状态变为参与详情后完成；报名意向为“不参加”的活动不执行报名。
- 已人工报名或已下线时不重复操作。
- 报名页出现未定义必填信息时，任务退回待用户补充，AI 不推测。
- 此次忽略：本次不报名，下次仍符合条件时继续提示。

【待补充】活动报名页可能出现的字段清单、各字段数据来源和填写策略；是否提供“以后不再提醒”及其关闭范围尚未确认。
...
| 14 | 商家经营分 | 开业时间、装修时间的诊断条件、字段映射、任务和执行规则待补充 | 对应评分任务无法执行 |
| 15 | 评价回复 | 文案长度、敏感词、用户编辑能力未定义 | 提交交互不完整 |
| 16 | 活动报名 | 报名页动态必填字段策略及长期忽略未定义 | 遇到新字段只能退回补充 |
