...
- 展示将要写入抖音的字段和用户可编辑内容。
- 不展示抖音来客不存在的字段。
- 任务详情与所选任务一一对应，展示该任务的来源数据与差异和建议写入字段；各节点任务详情的具体展示内容以第 18 章对应“任务展示 / 任务详情核对”为准（早餐份数修正任务不展示来源数据与差异，直接展示本次不通过房型与修改内容）；找店指引任务详情按第 18.2.5 节先选择指引方式，再按路线节点或指引视频填写；「中差评回复」任务按第 18.12.1 节逐条确认回复内容。
- 用户未确认前不得执行。
