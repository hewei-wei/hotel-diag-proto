...
1. 名称一致自动对应；名称不同但可可靠匹配时展示建议关系供用户确认；不能可靠匹配时必须先由用户选择对应抖音房型或“没有对应房型”；名称相同但床型、面积明显不同的同名房型需人工复核后才能对应。
2. 对应关系未确认前不得修改或新增。
...
- 已关联日历房、团购商品的抖音房型允许改名；改名后商品关联相应修改名称展示。
- 楼层、面积默认按区间写入，不取中间值。
...
【待补充】房型设施完整映射字典；房型图片数量和规格；“可靠匹配”的判断标准。
