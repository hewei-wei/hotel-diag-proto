...
- 检查项状态为六种：采集失败（抖音来客）、采集失败（携程）、采集失败（抖音来客、携程）、采集成功＋通过、采集成功＋不通过；抖音来客与携程均采集成功时按规则展示“采集成功＋通过”或“采集成功＋不通过”。状态涉及抖音来客采集失败时，右侧提示“请前往抖音来客检查此项是否符合标准”；仅携程采集失败且所采集数据非价格数据的，右侧提供“重试”按钮，点击后对该检查项重新发起携程采集（转圈样式状态），最多重试 5 次，超过 5 次后按钮置灰并提示“采集异常，请联系技术管理员”，重试成功后按采集结果更新为通过或不通过。
...
- 同一酒店每 24 小时内最多可发起 1 次诊断；焦点区展示剩余可诊断次数，24 小时内已诊断 1 次后“开始诊断”“重新诊断”置灰。点击“重新诊断”直接在当前诊断结果的基础上重新开始一次完整诊断，无需回到待开始状态；重新诊断执行期间，尚未重新检查到的步骤和检查项保留展示上一次诊断的结果（标注“上次”），重新检查完成后更新为本次结果；“重新诊断”右侧固定提示“每 24 小时只能诊断 1 次，请合理使用”。跨过 24 小时窗口后自动恢复可诊断。
...
- 同一酒店每 24 小时内最多可发起 1 次诊断；焦点区展示剩余可诊断次数，24 小时内已诊断 1 次后“开始诊断”“重新诊断”置灰，未开始时展示“24 小时内已诊断 1 次，达到诊断上限”；“重新诊断”右侧固定备注“每 24 小时只能诊断 1 次，请合理使用”。点击“重新诊断”直接在当前诊断结果的基础上重新开始一次完整诊断，无需回到待开始状态；重新诊断执行期间，尚未重新检查到的步骤和检查项保留展示上一次诊断的结果（步骤标注“上次通过/上次不通过”，检查项标注“上次通过/上次不通过”），重新检查完成后更新为本次结果。跨过 24 小时窗口后自动恢复可诊断。
...
- 检查项状态为“采集失败（抖音来客）”“采集失败（携程）”“采集失败（抖音来客、携程）”“采集成功＋通过”“采集成功＋不通过”；采集失败涉及抖音来客的，状态右侧提示“请前往抖音来客检查此项是否符合标准”；仅携程采集失败且非价格数据的，状态右侧提供“重试”按钮（转圈样式重新采集，最多 5 次，超过后按钮置灰并提示“采集异常，请联系技术管理员”）。该步骤生成的任务在步骤备注面板下方“本步骤已生成任务”区逐行展示，可在诊断任务列表中进入该任务详情。
...
- 检查项覆盖六种状态展示；抖音来客采集失败含“请前往抖音来客检查此项是否符合标准”提示，携程采集失败（非价格数据）支持重试且最多 5 次，超过后按钮置灰并提示“采集异常，请联系技术管理员”。
- 每个步骤完成时展示该步骤的通过数、不通过数、采集失败数和生成任务数；全部完成后展示本次诊断五项统计并可查看检查项详情、跳转诊断任务列表。
- 诊断完成后，该酒店的最近一次诊断完成时间更新。
- 同一酒店每 24 小时内最多可诊断 1 次；24 小时内已诊断 1 次后“开始诊断”“重新诊断”置灰；“重新诊断”右侧固定提示“每 24 小时只能诊断 1 次，请合理使用”；点击“重新诊断”直接重新开始一次完整诊断，执行期间尚未重新检查的步骤和检查项展示上一次诊断结果；跨过 24 小时窗口后自动恢复可诊断。
