...
- 商品搭配分为住宿、餐食、礼遇三部分：住宿从已配置的预售房型中选择，一个商品仅搭配一个预售房型；餐食按餐食类型（早餐 / 其它）与份数填写；礼遇填写可免费提供的项目。
- 套餐图展示预填图片并允许上传替换；携程无对应图片时标记为必须由用户上传。
- 价格核对项的建议售价为可修改的预填输入框，不展示计算逻辑；名称核对项展示「当前名称 → 建议名称」，建议名称可修改。
- 预约规则与消费规则（最早入住时间、最晚退房时间）为两个独立字段，分别展示与填写。
- 不可入住日期选择「节假日不可用」时覆盖全年全部节假日区间，非仅最近节假日；节假日团购商品不选择节假日不可用。
