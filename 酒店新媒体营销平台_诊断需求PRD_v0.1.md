...
1. 先选酒店，再诊断或执行。
2. 酒店数据、授权、诊断、任务、忽略配置和校验结果按 `hotel_id` 隔离。
3. 每家酒店分别授权抖音来客与携程官网；两个授权均有效才允许诊断或执行。酒店列表中的“抖音授权”为独立展示字段，不影响诊断。
...
- 新增酒店。
- 将酒店分配给运营人员。
- 在用户管理中维护账户：新增、编辑、禁用、删除、重置密码，并为账户绑定角色。
- 通过角色管理为角色绑定菜单、按钮、接口三类资源；内置超级管理员角色拥有全部资源。
- 在菜单管理中维护资源树（菜单、按钮、接口），资源开关即时写回权限状态。
- 新增运营账户，支持新增多个。
- 为每个运营账户配置酒店，配置后的酒店在该账户登录后的酒店管理页面即可查看和诊断。
- 不受权限限制，可访问全部菜单、按钮与全部诊断任务。
...
- 抖音来客授权。
- 携程官网授权。
- 抖音授权状态展示：酒店列表按酒店独立展示抖音授权状态，该字段不参与诊断前置条件。
- 手动诊断、每周诊断、每月诊断。
- 已获授权酒店的一键诊断；每家酒店仍独立生成诊断上下文和任务。
- 诊断任务查看与确认。
- AI 自动执行与结果校验。
- 线下任务的完成记录。
- 已处理诊断记录（诊断任务页内按诊断次数分组展示）。
- 用户管理：账户全生命周期管理（新增、编辑、禁用、删除、重置密码），并为运营账户配置酒店。
- 角色管理与菜单管理：角色绑定菜单、按钮、接口三类资源；资源树统一维护，资源开关即时生效。
- 我的信息：账户信息查看与修改密码（改密后强制重新登录）。
- 新增酒店：输入酒店名称即可新增，系统自动分配酒店ID，默认抖音来客、抖音授权和携程官网均为未授权。
- 全部 23 个诊断节点的诊断执行。
...
用户管理、角色管理、菜单管理位于“系统管理”分组，与酒店管理平级，仅管理员可见、可进入；“我的信息”为左侧导航一级菜单，全部账户可见、可进入。运营账户的左侧导航不展示系统管理分组。平台不提供切换账号功能：登录账户由管理员在用户管理中统一创建，右上角账号区仅展示当前登录账户的手机号与角色。


酒店管理页承载逐酒店的抖音来客与携程官网授权入口。左侧导航固定提供“酒店管理”菜单和“我的信息”一级菜单，管理员账号另展示“系统管理”分组（含“用户管理”“角色管理”“菜单管理”）；运营人员点击“诊断”进入对应酒店的二级诊断页面（诊断工作台），全部诊断在诊断工作台内按步骤完成。进入具体酒店后，右侧内容区顶部展示二级导航：返回酒店管理入口、当前酒店名称和“诊断工作台”“诊断任务”页签，酒店相关页面通过二级导航切换。
...
- 酒店首次分配给运营人员后，抖音授权默认为“未授权”。
- 未完成双授权时，不允许进入该酒店诊断或执行 AI 任务；抖音授权状态不影响诊断入口。

### 7.3 酒店列表展示

酒店管理页由全局框架、页面头部、前置条件提示、检索筛选和酒店列表五部分组成。左侧导航在各页面保持同一结构：品牌区、“酒店管理”菜单和“系统管理”分组；进入具体酒店后，酒店相关页面（诊断工作台、诊断任务）通过右侧内容区顶部的二级导航切换，二级导航持续展示当前酒店名称，并提供返回酒店管理页的入口。酒店管理页全部元素如下：

| 区域 | 元素 | 展示与交互要求 |
|---|---|---|
| 左侧导航 | 品牌区 | 展示平台标识、平台名称“酒店新媒体营销平台”和副标题“酒店智能运营工作台” |
| 左侧导航 | 导航菜单 | 固定展示“酒店管理”入口，右侧内容区处于酒店相关页面时该入口高亮 |
| 左侧导航 | 系统管理 | 展示“系统管理”分组与“用户管理”“角色管理”“菜单管理”入口，仅管理员账号展示该分组 |
| 顶部栏 | 账号区 | 位于页面右上角，展示当前账号头像、手机号和角色；仅作展示，不提供切换账号入口 |
| 二级导航 | 酒店二级页签 | 进入具体酒店后，在顶部栏下方展示：返回酒店管理入口、当前酒店名称和门店副名称，以及“诊断工作台”“诊断任务”页签；一个页签对应一个页面，当前页签高亮，“诊断任务”页签展示待处理任务数角标 |
| 页面头部 | 标题 | 展示“酒店管理” |
| 页面头部 | 新增酒店按钮 | 页面头部右侧展示“新增酒店”按钮；点击弹出新增弹窗，输入酒店名称确认后新增酒店，抖音来客、抖音授权和携程官网默认未授权；按钮受菜单管理中对应按钮资源的权限控制 |
...
| 酒店列表 | 抖音授权 | 展示抖音授权状态：已授权、未授权或已失效；该状态仅作展示，不影响“诊断”按钮可用性 |
...
3. “抖音授权”同样适用上述状态口径，按酒店独立维护；该字段仅作状态展示，不参与诊断与 AI 执行的前置校验。
...
酒店管理页页面头部提供“新增酒店”按钮：点击后弹窗输入酒店名称即可新增；新增酒店默认抖音来客、抖音授权和携程官网均为未授权，需完成授权后才可诊断。“新增酒店”按钮受菜单管理中对应按钮资源的权限控制。

已有酒店固定展示两个操作按钮，状态矩阵如下：
...
按钮状态实时取当前酒店的双平台授权结果。任一平台当前无效时，“诊断”置灰且不可点击。“诊断”按钮可用性仅由抖音来客与携程官网两个授权状态决定，抖音授权状态变化不改变按钮状态。
...
- 页面顶部为“本次诊断进度”焦点区：突出展示诊断状态说明、进度条和检查数、采集失败数、通过数、不通过数、生成任务数五项统计，并提供“开始诊断”入口，进入页面即可直接点击开始诊断；诊断执行中该区实时更新并展示当前执行到第几步，诊断完成后提供“查看任务”“重新诊断”入口，各统计项可点击查看对应检查项详情。
...
- 检查项状态为六种：采集失败（抖音来客）、采集失败（携程）、采集失败（抖音来客、携程）、采集成功＋通过、采集成功＋不通过；抖音来客与携程均采集成功时按规则展示“采集成功＋通过”或“采集成功＋不通过”。状态涉及抖音来客采集失败时，右侧提示“请前往抖音来客检查此项是否符合标准”；仅携程采集失败且所采集数据非房间相关数据的（房间相关数据指房型名称、房型设施信息、床型信息、楼层信息、价格信息等房型/价格类数据，下同），右侧提供“重试”按钮，点击后对该检查项重新发起携程采集（转圈样式状态），最多重试 5 次，超过 5 次后按钮置灰并提示“采集异常，请联系技术管理员”，重试成功后按采集结果更新为通过或不通过；所采集数据为房间相关数据的，不提供重试按钮。状态以彩色底色标签醒目展示：采集成功为绿底白字，通过为浅绿底绿字，不通过为红底白字，采集失败为橙底白字；抖音来客采集失败提示换行展示在状态下方，以浅橙底色突出显示。
- 步骤备注面板在检查项列表下方提供“本步骤已生成任务”区，任务按行独立展示：每行展示任务名称和对应检查项，可在诊断任务列表中进入该任务的详情页处理；联合诊断节点多项不通过合并为一个任务，在该区展示为一条。
- 每个步骤完成时在步骤行展示该步骤的通过数、不通过数、采集失败数和生成任务数；全部步骤完成后在焦点区汇总展示本次诊断的五项统计：检查数、采集失败数、通过数、不通过数、生成任务数，各统计项可点击，点击后在弹窗中查看对应检查项明细（检查项名称、所属步骤与当前状态），点击“生成任务数”跳转诊断任务列表。
- 全部步骤完成后，更新该酒店的最近一次诊断完成时间。
- 同一酒店诊断频率双重限制：每 24 小时内最多可发起 1 次诊断，且近 7 天内最多可诊断 2 次，达到任一上限即不可再诊断；焦点区展示剩余可诊断次数（取两者较低值），达到上限后“开始诊断”“重新诊断”置灰。点击“重新诊断”直接在当前诊断结果的基础上重新开始一次完整诊断，无需回到待开始状态；重新诊断执行期间，尚未重新检查到的步骤和检查项保留展示上一次诊断的结果（标注“上次”），重新检查完成后更新为本次结果；“重新诊断”右侧固定提示“7天内只能诊断2次，每 24 小时只能诊断 1 次，请合理使用”。跨过限制窗口后自动恢复可诊断。
...
任务处理状态仅针对单次诊断生成的任务，分为“待处理”“已处理”两级；“已处理”任务同时展示执行状态：执行中、执行成功、执行失败、已忽略。

```mermaid
stateDiagram-v2
    [*] --> 待处理
    待处理 --> 执行中: 用户提交或确认AI执行（转为已处理）
    待处理 --> 已忽略: 此次忽略，二次确认后（转为已处理）
    执行中 --> 执行成功: 执行且复核通过
    执行中 --> 执行失败: 执行或复核失败
    执行失败 --> 执行中: 按已确认数据重试
```
...
- 此次忽略：点击后弹出二次确认弹窗，确认后关闭本次任务并转为已处理，任务状态为“已忽略”；下次诊断仍检查该节点。
...
- 展示将要写入抖音的字段和用户可编辑内容。
- 不展示抖音来客不存在的字段。
- 任务详情与所选任务一一对应，各节点任务详情的具体展示内容以第 18 章对应“任务展示 / 任务详情核对”为准；找店指引任务详情按第 18.2.5 节先选择指引方式，再按路线节点或指引视频填写；「中差评回复」任务按第 18.12.1 节逐条确认回复内容。
- 用户未确认前不得执行。
...
- 待处理列表字段：任务名称、任务路径、任务诊断时间、操作。操作包含“去处理”“此次忽略”：“去处理”进入任务详情确认执行；“此次忽略”点击后弹出二次确认弹窗，确认后任务转为已处理。
- 已处理按诊断次数分组展示历史诊断记录，每次诊断一行，按诊断时间倒序排列；每行字段为：任务ID（后端生成、全局唯一）、诊断名称（“第x次诊断（YYYYMMDD）”，x 为该酒店的诊断次序，括号内为诊断日期）、生成任务数、执行中数量、执行成功数、执行失败数、已忽略数、诊断时间。
- 诊断记录默认收起，当次（最新一次）诊断默认展开；点击行展开或收起该次诊断的任务明细，逐条展示任务名称、任务路径、任务状态和“详情”按钮；执行失败的任务展示失败原因。
- 最新一次诊断完成后即在已处理生成一条诊断记录，任务尚未处理时任务明细均为“待处理”状态；开始处理后任务状态变为执行中、执行成功、执行失败、已忽略。执行中仅出现在最新一次诊断记录中，历史诊断记录中的任务均为终态（执行成功、执行失败、已忽略）。
- “详情”打开该任务的处理详情，只读展示任务处理时的数据快照，不可编辑、不提供操作按钮。
- 已处理页签提供任务状态筛选（全部、执行中、执行成功、执行失败、已忽略，只展示状态名称、不展示数量）和任务名称搜索框；筛选与搜索共同生效，命中的诊断记录自动展开、未命中的隐藏，无匹配任务时展示空态提示。
...
- 已处理诊断记录在诊断任务页“已处理”页签内展示，每次诊断一行，按诊断时间倒序排列（最新一次在最上方）。
- 记录字段为：任务ID、诊断名称、生成任务数、执行中数量、执行成功数、执行失败数、已忽略数、诊断时间。
- 任务ID 由后端生成、全局唯一；诊断名称为“第x次诊断（YYYYMMDD）”，x 为该酒店的诊断次序。
- 生成任务数为该次诊断生成的任务总数；执行中数量、执行成功数、执行失败数、已忽略数为该次诊断各执行状态的任务数。最新一次诊断完成后、任务尚未处理时，任务均为待处理状态，各执行状态数量为 0，随处理进展更新。
- 展开某次诊断后逐条展示任务明细：任务名称、检查路径、任务状态（待处理、执行中、执行成功、执行失败、已忽略）和“详情”按钮；执行失败的任务展示失败原因。
- “详情”只读展示该任务处理时的数据快照。
...
| 抖音无对应字段 | 不诊断、不展示、不写入 |
| 页面加载或采集失败 | 展示对应“采集失败”状态；涉及抖音来客的提示“请前往抖音来客检查此项是否符合标准”；仅携程且所采集数据非房间相关数据的提供“重试”（最多 5 次，超过后置灰并提示“采集异常，请联系技术管理员”），房间相关数据不提供重试；不把节点判为通过 |
...
- 新分配酒店的抖音来客、抖音授权和携程官网初始状态均为未授权；示例酒店覆盖双授权有效、单平台已失效、单平台未授权和双平台未授权状态。
- 列表列名依次为“酒店名称”“酒店ID”“抖音来客授权”“抖音授权”“携程官网授权”“最近一次诊断完成时间”“操作”。
- 酒店ID 为酒店唯一标识，由后端从用户中心直接获取，新增酒店时由系统自动分配。
- 每行操作区固定展示两个按钮。任一平台无效时展示“去授权＋置灰诊断”；双平台有效时展示“更新授权＋高亮诊断”。
- “去授权”和“更新授权”均打开当前酒店的双平台授权弹窗；“诊断”进入当前酒店的二级诊断页面（诊断工作台）。
- 左侧导航固定展示品牌区、“酒店管理”菜单、“我的信息”一级菜单和“系统管理”分组（仅管理员），各页面保持同一结构，右上角展示账号区（仅展示当前账号，不提供切换）。
- 进入具体酒店后，右侧内容区顶部展示二级导航：返回酒店管理入口、当前酒店名称和门店副名称，以及“诊断工作台”“诊断任务”页签；“诊断任务”页签展示待处理任务数角标。
- 授权入口位于对应酒店行。
- 页面支持酒店名称 / 酒店ID 搜索及“全部酒店 / 近 7 天已诊断 / 近 7 天未诊断”筛选，筛选均按最近一次诊断完成时间计算。
...
- 焦点区实时展示五项统计：检查数、采集失败数、通过数、不通过数、生成任务数；每项统计可点击查看对应检查项详情（检查项名称、所属步骤与当前状态），点击“生成任务数”跳转诊断任务列表。
- 同一酒店诊断频率双重限制：每 24 小时内最多可发起 1 次诊断，且近 7 天内最多可诊断 2 次，达到任一上限即不可再诊断；焦点区展示剩余可诊断次数（取两者较低值），达到上限后“开始诊断”“重新诊断”置灰，未开始时展示“24 小时内已诊断 1 次，达到诊断上限”或“7 天内已诊断 2 次，达到诊断上限”；“重新诊断”右侧固定备注“7天内只能诊断2次，每 24 小时只能诊断 1 次，请合理使用”。点击“重新诊断”直接在当前诊断结果的基础上重新开始一次完整诊断，无需回到待开始状态；重新诊断执行期间，尚未重新检查到的步骤和检查项保留展示上一次诊断的结果（步骤标注“上次通过/上次不通过”，检查项标注“上次通过/上次不通过”），重新检查完成后更新为本次结果。跨过限制窗口（24 小时或 7 天）后自动恢复可诊断。
...
- 检查项状态为“采集失败（抖音来客）”“采集失败（携程）”“采集失败（抖音来客、携程）”“采集成功＋通过”“采集成功＋不通过”；采集失败涉及抖音来客的，状态右侧提示“请前往抖音来客检查此项是否符合标准”；仅携程采集失败且所采集数据非房间相关数据（房型名称、房型设施信息、床型信息、楼层信息、价格信息等之外）的，状态右侧提供“重试”按钮（转圈样式重新采集，最多 5 次，超过后按钮置灰并提示“采集异常，请联系技术管理员”）；所采集数据为房间相关数据的，不提供重试按钮。状态以彩色底色标签醒目展示（采集成功绿底白字、通过浅绿底绿字、不通过红底白字、采集失败橙底白字），抖音来客采集失败提示换行展示在状态下方并突出显示。该步骤生成的任务在步骤备注面板下方“本步骤已生成任务”区逐行展示，可在诊断任务列表中进入该任务详情。
- 步骤完成时在步骤行展示通过数、不通过数、采集失败数和生成任务数。
- 全部步骤完成后，该酒店的最近一次诊断完成时间同步更新。
- 诊断和任务只属于当前酒店。

### 15.4 诊断任务页

- 页面按“待处理”“已处理”两个页签展示单次诊断生成的任务；“已处理”页签展示已处理诊断记录，每次诊断一行，按诊断时间倒序排列。
- 记录行字段为：任务ID（后端生成、全局唯一）、诊断名称（第x次诊断（YYYYMMDD））、生成任务数、执行中数量、执行成功数、执行失败数、已忽略数、诊断时间。
- 记录默认收起，最新一次诊断默认展开；展开后逐条展示该次诊断的任务名称、任务路径、任务状态（待处理、执行中、执行成功、执行失败、已忽略）和“详情”按钮，执行失败任务展示失败原因；“详情”只读展示该任务处理时的数据快照。诊断完成后、任务尚未处理时，明细任务均为待处理状态。
- 工具栏提供任务状态筛选（全部、执行中、执行成功、执行失败、已忽略，只展示状态名称、不展示数量）与任务名称搜索框。

### 15.5 用户管理页

系统管理采用“账号 — 角色 — 资源”三层关系：账户绑定角色（多对多），角色绑定菜单、按钮、接口三类资源（多对多），账户的可用权限取其全部角色的资源并集；菜单资源控制导航显隐，按钮资源控制操作显隐，接口资源约束后端调用。诊断任务节点是业务对象，不是权限资源，不进入资源树。

- 入口位于左侧导航“系统管理”分组，仅管理员可进入；运营账户左侧导航不展示该分组。
- 页面由查询条件区和数据列表组成：查询条件含“手机号”关键词与“账户状态”筛选，提供“查询”“重置”按钮；列表标题为“数据列表”，底部展示“共 X 条”与分页。
- 列表列依次为：手机号、角色、账户状态（开关）、创建时间、更新时间、操作。
- 页面头部提供“新增账户”按钮，弹窗字段为：手机号、密码、角色（多选）、账户状态。校验规则：
  - 手机号必填、为 11 位有效手机号、全局唯一，是登录凭证与展示标识，创建后不可修改。
  - 密码由 8-16 位数字、大小写字母组成，不能包含空格。
  - 角色至少选择一个，可选范围不含内置超级管理员角色。
- 行操作提供：编辑（角色、账户状态；手机号不可修改、密码不回显）、配置酒店、重置密码、删除。
- 内置管理员账户展示“内置账户”标签，不可禁用、不可删除。
- 删除、重置密码均弹出二次确认弹窗；重置密码统一重置为初始密码 A1234567，成功提示“重置成功，初始密码为 A1234567”。
- 禁用账户即时生效：被禁用账户无法登录；人员离职优先禁用保留记录，删除仅用于误建账户。
- “配置酒店”弹窗逐酒店勾选，展示酒店名称和门店副名称；保存后配置的酒店出现在该账户登录后的酒店管理页面，可查看和诊断（数据权限按账户维度配置）。
- 示例账户覆盖三类形态：内置管理员账户（绑定内置超级管理员角色）、配置多家酒店的运营账户（绑定运营管理员角色）、仅配置单家酒店的店长账户（绑定酒店店长角色）。

### 15.6 角色管理页

- 入口位于左侧导航“系统管理”分组，仅管理员可进入。
- 列表列依次为：角色名称、状态（开关）、备注、资源权限（按“X 个菜单 · Y 个按钮 · Z 个接口”计数展示）、操作（编辑、删除）。
- 页面头部提供“新增角色”按钮，弹窗字段为：角色名称（必填、唯一）、状态、备注、资源权限树（核心控件）。
- 资源权限树与菜单管理资源树结构一致，覆盖酒店管理、酒店管理-诊断工作台、酒店管理-诊断任务三个业务菜单，按菜单、按钮、接口逐项勾选：勾选菜单默认同时勾选其下全部按钮与接口子资源，子资源可单独取消或补充，不支持按菜单整体授权。系统管理资源仅管理员使用，不参与角色授权。
- 内置超级管理员角色展示“内置”标签，不可编辑、不可停用、不可删除，拥有全部资源。
- 角色被账户引用时不可删除，并提示已关联账户；未被引用的角色删除前弹出二次确认。
- 停用角色后不可再被新账户选择；已绑定该角色的账户不受影响。
- 示例角色覆盖四类权限形态：超级管理员（内置，全部资源）、运营管理员（三个业务菜单及全部按钮与接口，覆盖酒店接入、平台授权与诊断全流程）、酒店店长（酒店管理、诊断工作台、诊断任务三个菜单，不含新增酒店）、质检专员（诊断任务菜单与诊断记录详情按钮，只读查看诊断任务与已处理诊断记录、不可处理任务）。

### 15.7 菜单管理页（资源管理）

- 入口位于左侧导航“系统管理”分组，仅管理员可进入。
- 页面为树形资源列表，支持展开收起、按资源名称 / 编码搜索和资源类型筛选；列依次为：资源名称、资源类型、资源编码、资源路径、显示排序、操作。
- 资源分三类并以标签区分：菜单（前端路由）、按钮（页面操作）、接口（HTTP 路径）；按钮与接口作为子节点挂在所属菜单下，三类资源统一在一棵树中维护。
- 业务资源树：酒店管理、酒店管理-诊断工作台、酒店管理-诊断任务三个业务菜单，各自挂载按钮与接口资源，诊断任务菜单挂载“查询诊断记录”接口。系统管理菜单挂载用户管理、角色管理、菜单管理三个子菜单及各自接口。
- 资源开关即时写回权限状态：关闭菜单资源后账户导航不再展示该菜单；关闭按钮资源后对应按钮隐藏；关闭接口资源后后端拒绝对应调用。管理员自身不受资源开关限制。
- 页面头部提供“新增资源”按钮，弹窗字段为：上级资源（按钮和接口必须挂在菜单下）、资源类型、资源名称、资源编码（权限判定唯一键）、资源路径、显示排序（0-999）、资源描述。
- 行操作提供编辑与删除：业务内置资源不支持删除，可编辑或关闭权限；自定义资源删除前弹出二次确认。

### 15.8 我的信息页

- 入口位于左侧导航一级菜单“我的信息”，全部账户可见、可进入。
- 账户信息区只读展示：手机号、角色、账户状态。
- 修改密码区校验规则与用户管理一致；修改成功提示“修改成功，请重新登录”并强制退出登录，需使用新密码重新登录。
...
- 运营人员只能看到管理员分配的酒店。
- 新增账户后按其绑定角色的资源并集生效；内置超级管理员角色拥有全部菜单、按钮、接口资源。
- 示例角色与示例账户覆盖权限形态：角色含超级管理员、运营管理员、酒店店长与质检专员；账户含内置管理员、多酒店运营账户与单酒店店长账户。
- 按钮资源仅对勾选该按钮资源的角色所绑定账户生效：未勾选“新增酒店”按钮资源的账户，其酒店管理页不展示“新增酒店”按钮。
- 运营人员的酒店管理页仅展示并诊断被配置的酒店，未配置酒店不可进入。
- 运营人员未开通的菜单页面不可进入；“系统管理”分组对运营人员不展示。
- 管理员可见“系统管理”分组并可进入用户管理、角色管理、菜单管理；“我的信息”对全部账户在左侧导航一级菜单展示。
- 平台不提供切换账号功能；新增账户仅管理员可在用户管理中操作。
- 点击“新增酒店”输入酒店名称即可新增；新增酒店抖音来客、抖音授权与携程官网均展示未授权，授权列展示对应平台状态，“诊断”按钮置灰。
- 酒店列表展示“酒店ID”列（酒店唯一标识，由后端从用户中心获取，新增酒店自动分配）；搜索支持按酒店名称 / 酒店ID 检索。
- 菜单管理资源树统一维护菜单、按钮、接口三类资源；关闭菜单资源后导航不再展示，关闭按钮资源后按钮隐藏，关闭接口资源后接口调用被拒；管理员不受资源开关限制。
- 关闭某按钮权限后，运营人员对应页面不再展示该按钮；管理员不受按钮权限限制。
- 新分配酒店三项授权（抖音来客、抖音授权、携程官网）均显示未授权。
- 酒店列表展示“酒店名称”“抖音来客授权”“抖音授权”“携程官网授权”“最近一次诊断完成时间”“操作”六列。
- “抖音来客授权”和“携程官网授权”列展示对应平台的授权状态；“抖音授权”列展示抖音授权状态，该状态变化不影响“诊断”按钮可用性。
...
- 进入诊断工作台即可在页面首屏“本次诊断进度”区点击“开始诊断”；检查数、采集失败数、通过数、不通过数、生成任务数五项统计在该区实时更新，点击统计项可查看对应检查项详情，点击“生成任务数”跳转诊断任务列表。
- 诊断以检查路径为步骤可视化展示，步骤状态为未诊断、诊断中、诊断完成，并按路径顺序逐个执行。
- 检查项通过时不生成任务；检查项不通过时生成诊断任务，任务在步骤备注“本步骤已生成任务”区逐行展示，在诊断任务列表中进入该任务详情。
- 检查项覆盖六种状态展示，状态以彩色底色标签醒目展示（采集成功绿底白字、不通过红底白字、采集失败橙底白字），抖音来客采集失败提示换行并突出显示；抖音来客采集失败含“请前往抖音来客检查此项是否符合标准”提示，携程采集失败且所采集数据非房间相关数据（房型名称、房型设施信息、床型信息、楼层信息、价格信息等之外）的支持重试且最多 5 次，超过后按钮置灰并提示“采集异常，请联系技术管理员”，房间相关数据不提供重试按钮。
- 每个步骤完成时展示该步骤的通过数、不通过数、采集失败数和生成任务数；全部完成后展示本次诊断五项统计并可查看检查项详情、跳转诊断任务列表。
- 诊断完成后，该酒店的最近一次诊断完成时间更新。
- 同一酒店每 24 小时内最多可诊断 1 次，且近 7 天内最多可诊断 2 次，焦点区展示剩余可诊断次数（取两者较低值），达到任一上限后“开始诊断”“重新诊断”置灰；“重新诊断”右侧固定提示“7天内只能诊断2次，每 24 小时只能诊断 1 次，请合理使用”；点击“重新诊断”直接重新开始一次完整诊断，执行期间尚未重新检查的步骤和检查项展示上一次诊断结果；跨过限制窗口后自动恢复可诊断。
- 已处理诊断记录按次展示任务ID、诊断名称、生成任务数、执行中数量、执行成功数、执行失败数、已忽略数和诊断时间，默认收起、最新一次默认展开，展开后逐条展示任务明细与失败原因，“详情”只读展示处理数据快照；最新一次诊断完成后即在已处理出现该次记录，未处理任务以待处理状态展示，历史诊断记录不出现执行中状态。
- 任务不展示抖音不存在的字段。
- 携程无值时 AI 不自行补写。
- 任务、忽略状态和执行结果按酒店隔离。
...
- 待处理列表展示任务名称、任务路径、任务诊断时间和“去处理”“此次忽略”两个操作。
- 点击“此次忽略”弹出二次确认弹窗，确认后任务转为已处理，任务状态为“已忽略”；下次诊断仍检查该节点。
- 已处理按诊断次数分组展示历史诊断记录（任务ID、诊断名称、生成任务数、执行中数量、执行成功数、执行失败数、已忽略数、诊断时间），展开后逐条展示任务名称、任务路径、任务状态（待处理、执行中、执行成功、执行失败、已忽略）和“详情”（只读数据快照），执行失败任务展示失败原因。
...
- 系统管理采用“账号 — 角色 — 资源”三层关系：账户绑定角色（多对多），角色绑定资源（多对多），账户可用权限取其全部角色的资源并集。
- 资源统一为一棵树，分菜单、按钮、接口三类：菜单为页面路由，按钮为页面操作，接口为后端 API；按钮与接口挂在所属菜单下。
- 权限判定链：按账户的角色并集取得资源集合，前端按菜单资源渲染导航、按按钮资源控制操作显隐，后端按接口资源校验请求合法性；任一资源需同时满足角色已勾选且菜单管理中全局开启。
- 诊断任务节点是业务对象，不参与权限判定；管理员账户不受资源开关限制。

### 17.2 用户管理

- 用户管理为一级页面，位于“系统管理”分组，仅管理员可访问。
- 账户字段：手机号（登录凭证与展示标识，全局唯一，创建后不可修改）、角色（多选）、账户状态（启用 / 禁用）、创建时间、更新时间。
- 新增账户时设置初始密码；密码由 8-16 位数字、大小写字母组成，不能包含空格；编辑账户不回显密码，重置密码统一重置为初始密码 A1234567。
- 支持编辑、禁用、删除和重置密码；删除与重置密码均需二次确认；内置管理员账户不可禁用、不可删除。
- 管理员支持对每一个账户配置酒店；配置后的酒店在该账户登录后的酒店管理页面即可查看和诊断，未配置的酒店不出现在其酒店管理页面。

### 17.3 角色管理

- 角色管理为一级页面，位于“系统管理”分组，仅管理员可访问。
- 角色字段：角色名称（必填、唯一）、状态、备注、资源权限树。
- 资源权限树与菜单管理资源树结构一致，按菜单、按钮、接口逐项勾选：勾选菜单默认同时勾选其下全部子资源，子资源可单独调整。
- 内置超级管理员角色拥有全部资源，不可编辑、不可停用、不可删除；角色被账户引用时不可删除；停用角色不可再被新账户选择，已绑定账户不受影响。

### 17.4 菜单管理

- 菜单管理为一级页面，位于“系统管理”分组，仅管理员可访问，以树形列表统一维护菜单、按钮、接口三类资源。
- 资源字段：上级资源、资源类型、资源名称、资源编码（权限判定唯一键）、资源路径、显示排序（0-999）、资源描述；按钮和接口必须挂在菜单下。
- 资源开关即时写回权限状态并全站生效：菜单控制导航显隐，按钮控制操作显隐，接口约束后端调用；关闭接口资源后对应接口不可再被调用。
- 业务内置资源不支持删除，可编辑或关闭权限；自定义资源删除需二次确认。

### 17.5 我的信息

- “我的信息”为左侧导航一级菜单，全部账户可进入，只读查看手机号、角色和账户状态。
- 修改密码遵循统一密码策略：由 8-16 位数字、大小写字母组成，不能包含空格，二次输入一致；修改成功后需重新登录。
- 修改成功后强制重新登录，全部会话清空。
...
3. “此次忽略”遵循第 10.3 节；节点存在特殊关闭范围时，以节点规则为准。
...
- 任务：门店名称修改建议任务。任务展示建议名称（已预填、可修改），不展示名称公式。
...
- 任务页以单个板块「按抖音来客表单修改」呈现：表单与抖音来客「酒店政策」页面字段同构、五组全部可编辑；对比结论在板块开头提示（检出几处需修改、哪些分组一致），各差异字段在行内标注「由 xx 改为 xx」并按携程预填。
- 携程未采集到的政策字段，AI 不推测、保持当前值或留空，由用户选择完成后再提交（如儿童早餐的年龄 / 身高为提交前必选）。
- 任一分组必填项不完整时禁止提交。
- 提交：AI 统一填写并提交酒店政策，全部分组校验成功后完成。
- 此次忽略作用于整个联合节点，不拆分到单一政策。
...
- AI 点击「去设置」，读取抖音当前外显小图。
- 与携程图片素材池「外观」类别图片经图片识别逐一做一致性比对，无需比对位置；查到相同图片则通过，未查到则形成一条「外显小图替换建议任务」。
...
- 一致性比对采用图片识别判定；替换后图片可点选携程「外观」类别其他图片更换，或本地上传；确认后 AI 替换抖音来客外显小图，替换成功即任务完成，无需审核跟踪。
- 双列外显小图（搜索页候选图）不做检测和诊断，不判断、不补充，任务详情不展示该位置。
- 携程「外观」类别无图或张数不足时，直接提醒用户上传。
- 支持此次忽略；此次忽略只作用于当前酒店外显小图。
...
- 诊断范围：携程在售的全日房房型逐房型诊断；钟点房不在房型管理中诊断，仅在预售（18.9.1）和团购（18.10）中诊断。
...
- 已有房型差异任务：任务页仅展示字段差异与修改内容，逐字段展示抖音当前值、携程值和修改内容；楼层、面积按区间输入；床型修改内容按「床数量 / 床型 / 床宽」三个输入框展示（如携程为 1 张 1.8 米大床，展示「1张床 / 大床 / 1.8米」；携程为 2 张 1.35 米双人床，展示「2张床 / 双人床 / 1.35米」；携程超过两张床时床数量展示「多张床」），已按携程值预填、三个字段均可修改；网络服务按勾选；设施按勾选。一致字段不展示。
...
提交后 AI 只修改异常日期；部分失败时记录成功日期、失败日期和原因，不得完成。
...
2. 按价格由低到高取携程最低价、第二低价格、最高价三个档位；每个档位内全部同价房型均纳入，最终每档可超过一个房型。
3. 使用已确认的房型对应关系定位每个档位房型在抖音的适用房型。

**存在性与唯一性**

- 已存在对应适用房型的预售房型时，按原有规则逐项诊断（商品售卖时段、预售房型名称、房量房态—基础房量、房间数量），全部一致诊断通过。
- 不存在时该档位房型单独形成一条新增任务；多个档位缺失时分别生成任务，不合并。
- 同一适用房型只允许一个预售房型。

**钟点房检查**

- 检查抖音「预售房型」中是否存在钟点房；不存在时单独形成一条确认提示任务，提示文案：“检测到预售管理中没有钟点房，需要与酒店确认并建议去抖音来客系统新增钟点房。”
- 提示任务展示检查结果、平台提示与处理方式（与酒店确认后，去抖音来客系统新增钟点房），操作为「已完成」，不生成新增方案。
...
- 提示：“检测到携程XX房型没有在预售房型中，是否根据携程完善预售房型？详情信息如下。”（XX 为当前档位房型，档位为最低价 / 第二低价格 / 最高价）
...
1. 携程最低价房型的「2天1晚」团购。
2. 携程第二低价格房型的「三天两晚」和「2天1晚」两个团购。
3. 携程最高价房型的「三天两晚」团购。
4. 钟点房团购。

档位划分与 18.9.1 一致（最低价 / 第二低价格 / 最高价，同价房型全部纳入）。

**覆盖结果处理**

- 钟点房团购缺失时，在任务中单独展示提示卡片：“检测到团购管理中没有钟点房，需要与酒店确认并建议去抖音来客系统新增钟点房团购，方便为酒店引流。”提供处理意向选择：「去与酒店确认 / 暂不处理」，默认去与酒店确认。
- 其他房型团购缺失时，按 18.10.2 字段规则生成完整新增方案，可独立确认；部分缺失时只生成缺失方案。
- 覆盖要求全部满足时，按原有规则逐个诊断（商品搭配、实际售价、名称、间夜数、不可入住日期、预约及入住规则）。

**诊断范围与节假日**

1. AI 逐个点击团购商品“查看”，读取商品搭配、实际售卖价格、商品名称、间夜数、不可入住日期、预约及入住规则。
2. 未来 30 天有节假日：仅取最近一个节假日；涉及预售房型分别检查日常团购和节假日团购，缺失的按新增方案补充。
3. 一个团购商品只能搭配一个预售房型；同一预售房型可分别有日常和节假日团购，二者名称、价格必须不同。
...
- 「团购商品覆盖、价格与名称」不通过时生成一条「团购商品核对任务」，任务详情直接从「需新增的团购商品」开始展示，按以下分组展示：
  - 需新增的团购商品：按覆盖要求缺失项生成完整新增方案，字段按 18.10.2 预填，各方案可独立确认，默认均为确认新增；分组开头提示：“识别到抖音来客团购缺失{房型}{间夜数}预售 / {房型}{间夜数}套餐，下面将进行补充。”（按本次缺失项生成）。
  - 钟点房团购提示：无钟点房团购时单独展示提示卡片（文案见 18.10.1），提供处理意向选择「去与酒店确认 / 暂不处理」。
...
- AI 不生成或发布评价，不修改抖音字段。
- 任务详情操作为“已完成”“此次忽略”：用户线下执行后点击“已完成”，任务转为已处理（状态：执行成功）；不提供“提交”按钮。
